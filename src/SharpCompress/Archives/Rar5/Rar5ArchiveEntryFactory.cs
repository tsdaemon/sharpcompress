...
{
    internal static class Rar5ArchiveEntryFactory
    {
        ...
        internal static IEnumerable<Rar5ArchiveEntry> GetEntries(Rar5Archive archive,
                                                                IEnumerable<RarVolume> rarParts)
        {
            foreach (var groupedParts in GetMatchedFileParts(rarParts))
            {
                yield return new Rar5ArchiveEntry(archive, groupedParts);
            }
        }
    }
}